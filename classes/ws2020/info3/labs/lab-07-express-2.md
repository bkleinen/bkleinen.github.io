...
I've added create, update and delete actions to provide you with a full example.
...
## Step 7: Add an API Endpoint and use it

In order to toggle the done / not done state on my todo items, I do not want to
reload the entire page. Therefore, I introduce an API endpoint which enables me
...
To set up end-to-end tests for follow the [Test Express with Jest Tutorial]:


### 1. Install

- I had to switch to yarn to get a successful install (npm got caught in an endless recursive call). To do so, delete package-lock.yml and run yarn install. Then install
with yarn:
...
- already done if you used express-generator, the equivalent to server.js resides
in /bin/www

### 3. Create Tests.

Have at least one end-to end test checking that the correct data is shown on your page.

- [root test](https://github.com/htw-imi-info3/express-app04/blob/master/test/feature/root/root.test.js): just checks if get / returns a 200 HTTP Response Code.

- Check that todos in the db are listed: [todos/index](https://github.com/htw-imi-info3/express-app04/blob/master/test/feature/todos/index.test.js)

- be sure to call done()

I've [created an example that should fail but doesn't](https://github.com/htw-imi-info3/express-app04/commit/76a95c4d0cd42e9866460d1768a78772c11ad070) as the expectation is in
a callback function and jest will not wait for this to complete. The test succeeds,
there is, however, an indication that something is still running:
...
* factor out the database setup from app.js (mooved it to mongoose.js, import it in bin/www)
...
## Part 3 (Optional): Deploy the App to Heroku
